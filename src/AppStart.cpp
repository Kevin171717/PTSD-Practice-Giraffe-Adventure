...
void App::Start() {
    ...
    std::vector<std::string> countImage;
    countImage.reserve(4);
    for (int i = 2; i >= 0; --i) {
        countImage.emplace_back(GA_RESOURCE_DIR"/Image/Character/ball-" + std::to_string(i + 1) + ".png");
    }
    countImage.emplace_back(GA_RESOURCE_DIR"/Image/Character/ball-ok.png");

    m_Ball = std::make_shared<AnimatedCharacter>(countImage);
    m_Ball->SetZIndex(5);
